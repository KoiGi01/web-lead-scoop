...
- `extract-contacts`: website contact extraction, enrichment, and provider usage logging. Also returns a structured `websiteSignals` object built from the already-scraped homepage + contact-page HTML (no extra API calls), via the shared `supabase/functions/_shared/websiteSignals.ts` builder. **Enrich-mode profile discovery (2026-06-08): `discoverPublicProfiles` (Firecrawl `/v1/search`, 2 credits per result-bearing query, ~10/business) is the dominant Enrich cost and is largely redundant with Hunter (name+title+email+LinkedIn). It now (a) logs its spend to `api_usage_events` as a `firecrawl`/`search` op, and (b) runs ONLY as a Hunter-gated fallback — when Hunter returns 0 named contacts (or the homepage scrape fails) — instead of for every business. This cuts most Enrich Firecrawl burn with no quality loss. See `OPPORTUNITY_SIGNALS.md`. Must `supabase functions deploy extract-contacts` for it to take effect.**
