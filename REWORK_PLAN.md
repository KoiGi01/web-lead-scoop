...
Phase 4 status (2026-06-06): detection shipped and verified via PR #3 on branch `feat/opportunity-signal-detection`. Spec `docs/superpowers/specs/2026-06-05-opportunity-signal-detection-design.md`, plan `docs/superpowers/plans/2026-06-05-opportunity-signal-detection.md`. `npm.cmd run build` + 33 tests pass; per-part Codex verification confirmed no new external calls and an unchanged Places SKU. Decision: `weak_local_presence` keys off low reviews **and** weak rating (< 4.0) rather than the literal "no website" rule, which is near-dead in the live flow (extraction only runs on websited businesses); the spec was updated to match. Edge functions `search-places` (v35) and `extract-contacts` (v44) deployed to project `uoaxxxoqasczxcxygscy` on 2026-06-06; the shared `_shared/websiteSignals.ts` module bundled cleanly. Remaining before Phase 4 fully closes: persist signals to `saved_leads`, low-signal diagnostics, and a live opportunity-mode smoke test in the app.
