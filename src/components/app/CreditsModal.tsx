...
const CreditsModal = ({ open, onClose, onSelectBundle, loading }: CreditsModalProps) => {
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) onClose(); }}>
      <DialogContent className="overflow-hidden border border-[#EFEDE6]/10 bg-black p-0 sm:max-w-3xl">
            ...
            {PLANS.map((plan) => {
              ...
              return (
                ...
                >
                  ...
                  {showFounder ? (
                    <>
                      <span className="font-mono text-[10px] text-[#67645B] line-through">${plan.price}/mo</span>
                      <span className="text-2xl font-bold text-[#EFEDE6]">${plan.founderPrice}</span>
                      <span className="font-mono text-[9px] uppercase tracking-widest text-[#F5FF3D]">Founder 3 months</span>
                    </>
                  ) : (
                    <span className="text-2xl font-bold text-[#EFEDE6]">${plan.price}</span>
                  )}
                  <span className="mt-2 font-mono text-sm font-semibold text-[#F5FF3D]">{plan.credits} cr/mo</span>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {TOPUPS.map((bundle) => (
              <button
                ...
                className="relative flex min-h-[104px] flex-col items-center border border-[#EFEDE6]/10 p-3 transition-all hover:border-[#EFEDE6]/30 hover:bg-[#EFEDE6]/5 disabled:cursor-not-allowed disabled:opacity-60"
              >
