...
const stats = [
  { end: 34000, display: "34K+",  label: "LEADS GENERATED",   suffix: "+" },
  { end: 8200,  display: "8,200+",label: "BUSINESSES SCRAPED", suffix: "+" },
  { end: 47,    display: "47",    label: "COUNTRIES",           suffix: ""  },
...
const StatItem = ({ stat, visible, delay }: { stat: typeof stats[0]; visible: boolean; delay: number }) => {
  ...
  return (
    ...
    >
      <p
        className="font-black text-white tabular-nums tracking-tight leading-none"
        style={{ fontFamily: "'Space Mono', monospace", fontSize: "clamp(40px, 5vw, 64px)" }}
      >
        {visible ? stat.display : formatted}
      </p>
      <p className="label-mono tracking-[0.2em]">{stat.label}</p>
    </div>
  );
};
...
const StatsBar = () => {
  ...
  return (
    <div ref={ref} className="relative border-y border-white/[0.08] overflow-hidden" style={{ background: "#050505" }}>
      {/* Scanline texture */}
      ...
      <div className="relative mx-auto max-w-5xl px-4">
        <div className="grid grid-cols-2 md:grid-cols-4">
          {stats.map((s, i) => (
            <div key={s.label} className={i < stats.length - 1 ? "border-r border-white/[0.06]" : ""}>
